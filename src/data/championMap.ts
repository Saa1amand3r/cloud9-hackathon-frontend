...
export const championMap: Record<number, { name: string; key: string }> = {
...
export function getChampionName(id: number): string {
  return championMap[id]?.name ?? `Champion ${id}`;
}

export function getChampionKey(id: number): string {
  return championMap[id]?.key ?? String(id);
}

export function getChampionImageUrl(id: number): string {
  const key = getChampionKey(id);
  return `https://ddragon.leagueoflegends.com/cdn/${DDRAGON_VERSION}/img/champion/${key}.png`;
}

export function getChampionSplashUrl(id: number, skinNum = 0): string {
  const key = getChampionKey(id);
